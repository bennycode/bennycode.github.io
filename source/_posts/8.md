...
We need to add `webpack-karma` so that Karma can use `webpack` to preprocess files. This also requires us to include `webpack` as it is a peer dependency of `webpack-karma`:
...
This happens when you forgot to install `webpack`.

### Update Karma configuration

{% codeblock lang:js mark:6,11-13,17-19,22-24 karma.conf.js %}
