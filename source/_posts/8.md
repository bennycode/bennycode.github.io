...
Our tests will run in the browser so we cannot import code with a `require` (CommonJS syntax) anymore and need to use the `window` namespace:
...
A Karma configuration can be interactively created by running `npx karma init`. In our case we reuse the file paths of our `jasmine.json`. 
